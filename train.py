...
def get_ood_test_loaders(args):
    ...
    # /////////////// SVHN ///////////////
    svhn_data = dset.SVHN(root=f'{args.data_dir}/svhn/', split='test',
                     transform=transform_ood, download=True)
    svhn_loader = DataLoader(svhn_data, batch_size=args.batch_size, num_workers=4)

    # # /////////////// LSUN-R ///////////////
    # if args.dataset != 'stl10':
    #     lsun_data = dset.ImageFolder(root=f'{args.data_dir}/kirby_dataset/LSUN_resize', transform=transform_ood)
    # else:
    #     lsun_data = dset.ImageFolder(root=f'{args.data_dir}/kirby_dataset/SUN', transform=transform_ood)
    # lsun_loader = DataLoader(lsun_data, batch_size=args.batch_size, num_workers=4)

    # # /////////////// LSUN-C ///////////////
    # if args.dataset != 'stl10':
    #     lsun_crop_data = dset.ImageFolder(root=f'{args.data_dir}/kirby_dataset/LSUN_C', transform=transform_ood)
    # else:
    #     lsun_crop_data = dset.ImageFolder(root=f'{args.data_dir}/kirby_dataset/SUN',
    ...
    # # /////////////// Textures ///////////////
    # dtd_data = dset.ImageFolder(root=f'{args.data_dir}/kirby_dataset/dtd/images', transform=transform_ood)
    # dtd_loader = DataLoader(dtd_data, batch_size=args.batch_size, num_workers=4)

    # # /////////////// PLACE ///////////////
    # place_data = dset.ImageFolder(root=f'{args.data_dir}/kirby_dataset/Places', transform=transform_ood)
    # place_loader = DataLoader(place_data, batch_size=args.batch_size, num_workers=4)

    # # /////////////// iSUN ///////////////
    # isun_data = dset.ImageFolder(root=f'{args.data_dir}/kirby_dataset/iSUN', transform=transform_ood)
...
def get_ind_dataset(args):
    ...
    if args.dataset == 'stl10':
        ind_train_data = dset.STL10(f'{args.data_dir}', split='train', download=True, transform=transform_train)
        ind_test_data = dset.STL10(f'{args.data_dir}', split='test', download=True, transform=transform_train)
    elif args.dataset == 'cifar10':
        ind_train_data = dset.CIFAR10(f'{args.data_dir}', train=True, download=True, transform=transform_train)
        ind_test_data = dset.CIFAR10(f'{args.data_dir}', train=False, download=True, transform=transform_train)
    else:
        ind_train_data = dset.CIFAR100(f'{args.data_dir}', train=True, download=True, transform=transform_train)
        ind_test_data = dset.CIFAR100(f'{args.data_dir}', train=False, download=True, transform=transform_train)
    
    return ind_train_data, ind_test_data


def get_ood_train_dataset(args):
    valid_image_files = glob.glob(os.path.join(args.data_dir, args.dataset, '0/train', '*/*'))
    ...
    return OODDataset(valid_image_files, args.num_classes, transform=transform_ood)
...
def get_model(args):
    ...
    net.load_state_dict(torch.load(f'{args.weight_dir}/{args.dataset}_{args.arch}_pretrained_epoch_99.pt', map_location=args.device))
    net = net.eval()
...
def train_ood_detector(args, test_ood_loaders):
    ...
    for ood_name, ood_loader in test_ood_loaders.items():
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_dir', default='./pretrained', type=str)
